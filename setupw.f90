...
!hliuftc-------------------------
  ...
  use jfunc, only: jiter,last,jiterstart,miter, first
...
!hliuftc
  real(r_kind) presw,factw,dpres,ugesin,vgesin,rwgt,dpressave, ugesdif,vgesdif, dpres9
...
!hliuftc-----------------------------------------------------------START
  real(r_kind),dimension(-lftc:lftc) :: pftc, wftc, uftc0, vftc0
  real(r_kind) ugesftc, vgesftc, ugesopr, vgesopr, duftc,dvftc, lat0, &
               amverr1, amverr2, amverr3, ddd, ffact
  real(r_kind),dimension(nsig) :: uges, vges
  logical ifirst     ! local

!  ratio of STDV in O-B (FTC/OPER) definition level
  real(i_kind),parameter:: nlay=18, nreg=3
  logical noftc
  integer(i_kind) n8, k8, lnd, kl
  real(r_kind), save:: pave(nlay), erradj(nreg, nlay, 240:260)
...
!hliuftc-----------
   ifirst = .true.
    ...
    open(unit=18, file='cycleid.txt', status='old', form='formatted')
    read(18, '(a4)') cycleid
    close (18)
...
!    index information for data array (see reading routine)
  ...
  ipres=4     ! index of pressure   ln(pressure in cb)
...
!  --------- for pressure-coordinate  OBS --------------------------------
!    Process observations with reported pressure
!  --------- for pressure-coordinate  OBS --------------------------------
     else
!hliu      dpres: ln(pressure in cb)
        dpres = data(ipres,i)           
        presw = ten*exp(dpres)          
...
!  need to skip above operational AMVs processing to save CPUs
!--------------------------------------------------------------
!--------------------------------------------------------------
! AMV VarFTC operator
!--------------------------------------------------------------
 if(itype >=240 .and. itype <=260) then       ! AMVs only
!--------------------------------------------------------------
...
!         itype==245.or.itype==246.or.itype==247.or. &
 ...
 if( cycleid == 'anal' ) then         ! skip oebs / eomg step
! ------ interpolate UV guess profile to each AMV lat/lon/time --------------
!   for output of guess uv profiles at AMV locations only ------------
     call tintrp2a1(ges_u, uges, dlat,dlon,dtime,hrdifsig,nsig,mype,nfldsig)
     call tintrp2a1(ges_v, vges, dlat,dlon,dtime,hrdifsig,nsig,mype,nfldsig)
 endif


!------------- FTC p-grid relative to each AMV pressure (mb) -----------
!  pk= po - k δp, with δp = 25 hPa
!  k=0 is the AMV level, p=po; k=1 is one level up in elevation

   do k = -lftc, lftc
    pftc(k) = presw - k*dpftc           ! k=1 one level up in elevation
    pftc(k) = log(pftc(k)/r10)          ! convert to log(cb), prsltmp: log(cb)
   enddo

   do k= -lftc, lftc
        dpres9 = pftc(k)
        dpres9 = dpres9-log(psges)          ! psges in cb (0.1*mb)

!       Put obs pressure in correct units to get grid coord. number
        dpres9 =log(exp(dpres9)*prsfc)
        call grdcrd1(dpres9, prsltmp(1),nsig,-1)
 
!       Interpolate guess u and v to FTC p-grid.
 
        call tintrp31(ges_u, uftc0(k),dlat,dlon,dpres9,dtime, &
           hrdifsig,mype,nfldsig)
        call tintrp31(ges_v, vftc0(k),dlat,dlon,dpres9,dtime, &
           hrdifsig,mype,nfldsig)

        pftc(k) = dpres9
!    pftc and prsltmp(1) are also used later for intw/stpw minimization.

   enddo

! --------- average u/v on FTC p-grid ----------
    lat0 = data(ilate,i) 
    call defftc(itype, wftc, duftc, dvftc, ifirst, lat0, presw, noftc)

    if( mod(i,300) ==0) print*, 'FTCbias= ', duftc, dvftc, presw, lat0

   ugesftc = zero
   vgesftc = zero
   do k = -lftc, lftc
    ugesftc = ugesftc + uftc0(k) * wftc(k)
    vgesftc = vgesftc + vftc0(k) * wftc(k)
   enddo

! -------- add additive bias correction term --------
   ugesftc = ugesftc + duftc
   vgesftc = vgesftc + dvftc
...
!!hliuftc--------------AMV error tuning --------------------
!   adjust qcgross critia by the error adjustment due to FTC
!   to ensure the same AMV observations are assimilated.
...
!------------------------------------------------------------
  ...
  if( noftc) then
        ! NO FTC is applied to these types
   else            ! FTC operator for 245, 246, 247, 260 only
        lnd = 2
        if( lat0 < -20.0) lnd = 1
        if( lat0 >  20.0) lnd = 3
        k8 = minloc( abs(pave - presw), 1)
 
    if ( erradj(lnd, k8, itype) < 1.0 ) then         !! for (O-B)/error
...
!--------------- TMP -----------------------
! if( itype == 247 .and. lat0 < -5.0 ) then
!    factor = 1.0
! endif

! if(presw <=800.0 .and. presw >450.0) then    ! mid layer no FTC
!   factor = 1.0
! endif
...
!--------------- TMP -----------------------
     ...
     qcgross = qcgross / factor    !! consistent to read_satwnd.f90
      if( mod(i,300) ==0) print*, 'qcgross1= ', qcgross, factor
    endif
...
!hliuftc-------------------------------------------------
   ...
   allocate(my_head%wftc(-lftc:lftc),stat=istat)
   allocate(my_head%ftcij(8,-lftc:lftc),stat=istat)
   allocate(my_head%ftcwij(8,-lftc:lftc),stat=istat)
   if (istat/=0) then
     write(6,*)'allocate error for whead_wftc, istat=',istat
     stop
   end if
    ...
    do k = -lftc, lftc
! ------------ pftc is already in model level index ------------------
!  get model grid index of the guess field in interpolation

     call get_ijk(mm1,dlat,dlon,pftc(k),my_head%ftcij(:,k),my_head%ftcwij(:,k))
 
      do j=1,8
         my_head%ftcwij(j, k)=factw*my_head%ftcwij(j,k)
      end do
 
     my_head%wftc(k)= wftc(k)             ! FTC weights for each AMV
    end do     ! k loop
   end if
!hliuftc-----------------------------------------------END
...
!hliu      call nc_diag_metadata("Setup_QC_Mark",           sngl(bmiss)            )
...
!hliu      call nc_diag_metadata("Errinv_Adjust",           sngl(errinv_adjst)     )
...
! iqc=12      ! index of quality mark
! ier2=13     ! index of original-original obs error ratio
! iprvd=22    ! index of observation provider                      for qifn TMP
! isprvd=23   ! index of observation subprovider                   for qify TMP
! icat=24     ! index of data level category                       for ee TMP

!liuamvoutput----------------------------------------

 if (itype >=240 .and. itype <=260) then

 if( cycleid == 'anal' ) then
  if(first)then          ! 1st outloop only

  if( abs(errinv_final) > 0.001 .and. muse(i) ) then

! -----------  2022.06.11 allow 254 with ee=110 in output ------------------
    if( abs( data(ier2,i)) < 100.0 .and.  abs(data(icat,i)) < 120.0 ) then
!   if( abs( data(ier2,i)) < 100.0 .and.  abs(data(icat,i)) < 100.0 ) then
! -----------  2022.06.11 ---------------------------

   write(901+mype, '(2i5, 16f8.2)') &
                  itype, icsubtype(ikx), sngl(data(ilate,i)), sngl(data(ilone,i)), &
                  sngl(presw), sngl(data(ier2,i)), sngl(dtime-time_offset), &
                  sngl(data(iuob,i)), sngl(data(ivob,i)),  &
                  sngl(ugesopr), sngl(vgesopr), &
                  sngl(ugesftc), sngl(vgesftc), sngl(data(iqc,i)), &
                  sngl(data(iprvd,i)), sngl(data(isprvd,i)), sngl(data(icat,i)), &
                  sngl(psges*r10)

! meaning of above line  qifn,                 qify,                       ee
!                 station_id(3:5)

! this next entire GFS profile output can be skipped
   do k=1, nsig-24
   write(901+mype, '(3f7.2)') sngl(ten*exp(prsltmp(k))),sngl(uges(k)),sngl(vges(k))
   enddo

!---------------------------------------------------------------
! Output u/v guess at 25hPa grid around AMV location 
! The above outputs can be skipped if the below output works 
! It is much simpler to output the data in ascii than in netcdf 
!
   write(9901+mype, '(2i5, 16f8.2)') &
                  itype, icsubtype(ikx), sngl(data(ilate,i)), sngl(data(ilone,i)), &
                  sngl(presw), sngl(data(ier2,i)), sngl(dtime-time_offset), &
                  sngl(data(iuob,i)), sngl(data(ivob,i)),  &
                  sngl(ugesopr), sngl(vgesopr), &
...
! in current implementation, max lftc =5 is allowed due to the allocated arrays
   do k = -lftc, lftc         
   write(9901+mype, '(2f7.2)') sngl(uftc0(k)),sngl(vftc0(k))
   enddo
...
! ----- for output in netcdf files ----------
   amverr1 = data(iprvd,i)     ! qfin
   amverr2 = data(isprvd,i)    ! qfiy
   amverr3 = data(icat,i)      ! ee
   if( abs(amverr1) > 200.0)  amverr1 = 0.0
   if( abs(amverr2) > 200.0)  amverr2 = 0.0
   if( abs(amverr3) > 200.0)  amverr3 = 0.0

else           ! non-AMV winds to skip huge error marks
   ugesftc = zero
   vgesftc = zero
   amverr1 = 0.0
   amverr2 = 0.0
...
!hliu------ netcdf4 has limit of size issue, especially under mpi -------
!           can cause random  interruptions in setupw.f90
! ---------  netcdf files with O-B are read/input in enkf.x ----------

              call nc_diag_metadata("qifn",  sngl(amverr1))
!             call nc_diag_metadata("ee",    sngl(amverr3))
!
              call nc_diag_metadata("u_Observation",                              sngl(data(iuob,i))    )
              call nc_diag_metadata("u_Obs_Minus_Forecast_adjusted",              sngl(dudiff)          )
!   ----------the next is needed in enkf --------------------
...
!hliuftc--------------------------------------------------------------------------------
              call nc_diag_metadata("u_Forecast_OPER",     sngl(ugesopr) )
              call nc_diag_metadata("v_Forecast_OPER",     sngl(vgesopr) )

!             call nc_diag_metadata("u_Forecast_FTC",     sngl(ugesftc) )
!             call nc_diag_metadata("v_Forecast_FTC",     sngl(vgesftc) )
!hliuftc--------------------------------------------------------------------------------
...
!hliuftc------------------------------------------------------------------
  subroutine defftc(itype, wftc, duftc, dvftc, ifirst, lat0, presw, noftc)
!-------------------------------------------------------------------------
  implicit none
  integer(i_kind),parameter:: lftc0 = 12, nsat = 21, nreg = 3, nlev=3   ! 240-260
  TYPE :: ftc_coefs
     integer(i_kind) :: satype
     integer(i_kind) :: ireg
     integer(i_kind) :: ilev
     integer(i_kind) :: ndat
     real(r_kind) :: ombrmsd
     ...
     real(r_kind) :: olsrmsd
     real(r_kind) :: olsmean
     real(r_kind) :: olsgamma
     real(r_kind) :: olsdelta
     real(r_kind) :: olsdeltav
     real(r_kind) :: lmrmsd
     real(r_kind) :: lmmean
     real(r_kind) :: lmgamma
     real(r_kind) :: lmdelta
     real(r_kind) :: lmdeltav
     integer(i_kind) :: ss
     integer(i_kind) :: tt
     real(r_kind), dimension(-lftc0:lftc0) :: wi             ! from coef.csv file
  END TYPE ftc_coefs

TYPE(ftc_coefs) :: coef00
TYPE(ftc_coefs), save :: ftccoef(nreg, nlev, 240:260)

  integer(i_kind), intent(in):: itype
  logical, intent(in):: noftc
  real(r_kind),intent(in):: lat0, presw
  real(r_kind):: fff, phigh, plow, pmid
  real(r_kind):: f11, f12, f13, f21, f22, f23, ffa, ffb, ff1, ff2, &
                 xx1, xx2, xx3, xx4, xx, yy1, yy2, yy

  real(r_kind),dimension(-lftc:lftc),intent(out) :: wftc
  real(r_kind),intent(out):: duftc,dvftc

  integer(i_kind) nnn, k, n, lnd, lll, khigh, klow, lat1, lat2, lat3
  logical ifirst
...
! -------- set default ---------------------
   do n=1, nreg
   do k=1, nlev
    do j = 240, 260        ! AMV type loop
     ftccoef(n,k,j)%wi    = zero
     ftccoef(n,k,j)%wi(0) = one
     ftccoef(n,k,j)%lmdelta  = zero
     ftccoef(n,k,j)%lmdeltav = zero
    enddo
   enddo
   enddo
!---------------------------------------------------------------------
    open(unit=18, file='ftc_coef.txt', status='old', form='formatted')
! -----  store the reading (incomplete) sat tpes into ftccoef array

!   nsat + avg = nsat+1
    do j = 1, (nsat+1)*(nreg+1)*(nlev+1)     
     read(18,*, end=111) coef00
      nnn = coef00%satype
      lnd = coef00%ireg
      lll = coef00%ilev

    if( nnn>0 .and. lnd>0 .and. lll>0 ) then
     ftccoef(lnd, lll,nnn)%wi      = coef00%wi
     ftccoef(lnd, lll,nnn)%lmdelta = coef00%lmdelta
     ftccoef(lnd, lll,nnn)%lmdeltav= coef00%lmdeltav
    endif
...
!----------------------------------------------
   ...
   if( mype == 0) then
    do j=240, 260        ! AMV type loop
     print*, ' ftccoef1= ', j,  ( sngl(ftccoef(n,1, j)%wi), n=1, nreg)
     print*, ' ftccoef2= ', j,  ( sngl(ftccoef(n,2, j)%wi), n=1, nreg)
     print*, ' ftccoef3= ', j,  ( sngl(ftccoef(n,3, j)%wi), n=1, nreg)
    enddo
   endif
...
!-----------------------------
  lnd = 2            ! TR
 if( lat0 < -20.0) lnd = 1   ! SH
 if( lat0 >  20.0) lnd = 3   ! NH

    phigh = 450.0   ! interface upper
    plow  = 800.0   ! interface low

  if( presw <= phigh) then
   lll = 1        ! high
   else if( presw > phigh .and. presw <= plow) then
   lll = 2        ! mid
   else if( presw > plow) then
   lll = 3        ! low
  endif
...
if( noftc) then
   ...
   do k = -lftc, lftc
    wftc(k) = ftccoef(lnd, lll, itype)%wi(k)
   end do

    duftc  = ftccoef(lnd, lll, itype)%lmdelta
    dvftc  = ftccoef(lnd, lll, itype)%lmdeltav
...
! ---------- interpolation between regions -------------------
      ...
      lat3 = 3
! SH/TRO  -------------
      xx1 = -30.0   ! transition zone in deg
      xx2 = -10.0

   if(lat0 >xx1 .and. lat0 < xx2) then
    do k = -lftc, lftc
      f11 =  ftccoef(lat1, lll, itype)%wi(k)    ! SH high
      f12 =  ftccoef(lat2, lll, itype)%wi(k)
      wftc(k) = f11 + (f12-f11)*(xx-xx1)/(xx2-xx1)
    enddo
     if( mype == 0) print*, 'intcoef2 ', (wftc(k), k=-lftc, lftc), lat0,presw

! u bias term ----------
      f11 =  ftccoef(lat1, lll, itype)%lmdelta
      f12 =  ftccoef(lat2, lll, itype)%lmdelta
      duftc = f11 + (f12-f11)*(xx-xx1)/(xx2-xx1)

! v bias term ----------
      f11 =  ftccoef(lat1, lll, itype)%lmdeltav
      f12 =  ftccoef(lat2, lll, itype)%lmdeltav
      dvftc = f11 + (f12-f11)*(xx-xx1)/(xx2-xx1)
...
! NH/TRO  ----------------
      xx3 =  10.0
      xx4 =  30.0
   if(lat0 >xx3 .and. lat0 < xx4) then
    do k = -lftc, lftc
      f12 =  ftccoef(lat2, lll, itype)%wi(k)
      f13 =  ftccoef(lat3, lll, itype)%wi(k)
      wftc(k) = f12 + (f13-f12)*(xx-xx3)/(xx4-xx3)
    enddo
     if( mype == 0) print*, 'intcoef3 ', (wftc(k), k=-lftc, lftc), lat0,presw

! u bias term ----------
      f12 =  ftccoef(lat2, lll, itype)%lmdelta
      f13 =  ftccoef(lat3, lll, itype)%lmdelta
      duftc = f12 + (f13-f12)*(xx-xx3)/(xx4-xx3)

! v bias term ----------
      f12 =  ftccoef(lat2, lll, itype)%lmdeltav
      f13 =  ftccoef(lat3, lll, itype)%lmdeltav
      dvftc = f12 + (f13-f12)*(xx-xx3)/(xx4-xx3)
...
!------------ temporary -------------------------------
! Not apply FTC for AMV 247 in SHX due to larger bias
!------------ temporary -------------------------------
! if( itype == 247 .and. lat0 < -5.0 ) then
!  wftc    = zero
!  wftc(0) = one
...
!------- Not apply FTC in the mid-layer due to small samples --------
! if (lll == 2) then
!  wftc    = zero
!  wftc(0) = one
!  duftc  = zero
